...
- **ما في تسجيل ذاتي عام.** صاحب المنصة ينشئ المدرسة من `/superadmin` عبر Worker موثوق؛ قواعد Firestore نفسها
  بترفض إنشاء أي مدرسة أو حساب `admin` من المتصفح إطلاقًا. ويبقى
  (`scripts/create-school.mjs`) كـ fallback إداري عند الحاجة. بعد ما تُنشأ
...
### إنشاء مدرسة جديدة (المسار الأساسي)

1. سجّل دخول صاحب المنصة على `/superadmin`.
2. افتح `إضافة مدرسة جديدة` وأدخل اسم المدرسة وبيانات المدير.
3. أكد العملية بكلمة سر صاحب المنصة.
4. سلّم بيانات الدخول المؤقتة للمدير بأمان؛ أول دخول يجبره على تغيير كلمة السر.

### المسار الاحتياطي من الطرفية
