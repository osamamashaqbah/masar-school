...
النطاق الحالي: إنشاء الحسابات وإدارة دورة حياتها (`create-school-user` و`update-school-user`) وتسجيل التدقيق الموثوق (`audit-log`). كل المسارات عليها حدّ حافة لكل IP، وحدّ إضافي لكل فاعل موثّق.
باقي عمليات Bug 2 (الاستيراد الجماعي، الترفيع السنوي، المؤشرات، الإشعارات) لسا من جهة العميل —
ممكن تنتقل لهون بنفس الطريقة لاحقًا.
...
`deactivate` أو `activate` أو `reset-password`. إعادة تعيين كلمة السر ترجع كلمة مؤقتة
للمدير مرة واحدة، ويُطلب من المستخدم تغييرها عند أول دخول.
